...
async def main():
    ...
    game_over = False
    won = False
    waiting_to_start = True  # NEW: Satisfies the browser's User Action rule
    clock = pygame.time.Clock()

    while running:
        screen.fill(BLACK)

        # --- 1. START SCREEN (Fixes the "Loading" hang) ---
        if waiting_to_start:
            msg = "CLICK TO START"
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    running = False
                # Detection of any click or key wakes up the browser's engine
                if event.type == pygame.MOUSEBUTTONDOWN or event.type == pygame.KEYDOWN:
                    waiting_to_start = False

            pygame.display.flip()
            await asyncio.sleep(0)  # Keep the browser responsive
            continue
...
if __name__ == "__main__":
    asyncio.run(main())  # Required web entry guard